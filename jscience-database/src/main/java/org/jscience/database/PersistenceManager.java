...
public class PersistenceManager {
    ...
    public void register(Class<?> clazz) throws SQLException {
        ...
        for (Field field : fields) {
            if (field.isAnnotationPresent(Id.class) || field.isAnnotationPresent(Attribute.class)) {
                ...
                } else if (org.jscience.mathematics.numbers.real.Real.class.isAssignableFrom(type)) {
                    sql.append("DOUBLE");
                } else {
    ...
    private Object convertValue(Object val) {
        ...
        if (val instanceof org.jscience.util.identity.Identified) {
            Object idVal = ((org.jscience.util.identity.Identified<?>) val).getId();
            return convertValue(idVal);
        }
        if (val instanceof org.jscience.mathematics.numbers.real.Real) {
            return ((org.jscience.mathematics.numbers.real.Real) val).doubleValue();
        }
        if (val instanceof org.jscience.measure.Quantity) {
