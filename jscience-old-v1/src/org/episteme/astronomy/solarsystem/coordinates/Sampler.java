package org.episteme.astronomy.solarsystem.coordinates;
...
public abstract class Sampler implements skyview.Component {
	...
	String classString = "org.episteme.astronomy.solarsystem.coordinates.sampler."+base+"Sampler";
	Sampler samp;
	try {
	    samp = (Sampler) Class.forName(classString).newInstance();
	} catch (Exception e) {
	    try {
	        samp = (Sampler) Class.forName("org.episteme.astronomy.solarsystem.coordinates.sampler."+type).newInstance();
	    } catch (Exception f) {
		try {
