...
public class YahooChangeSource extends ParserCallback implements ChangeSource {
    ...
    public Money getConverted(Money amount, Currency target)
        ...
        String u = "http://finance.yahoo.com/m5?a=" + amount.getValue().doubleValue() + "&s=" +
            amount.getCurrency().getCode() + "&t=" + target.getCode();
        tableCurrent = 0;
        rowCurrent = 0;
        cellCurrent = 0;
        resultRate = -1;
        ...
        if (resultRate == -1) {
            throw new UnavailableDataException("Rate not found in Yahoo response.");
        }

        return Money.valueOf(amount.getValue().doubleValue() * resultRate, target);
    }
    ...
    public void handleText(char[] data, int pos) {
        if (found == true) {
            ...
            resultRate = Double.parseDouble(new String(cleanFloat.toByteArray()));
            found = false;
        }
    }
}
