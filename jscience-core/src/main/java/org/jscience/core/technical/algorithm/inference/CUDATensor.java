...
public class CUDATensor<T> implements Tensor<T> {
    ...
    public Tensor<T> multiply(Tensor<T> other) {
         // Hadamard (element-wise) product via GPU→CPU roundtrip
         // (cuBLAS has no element-wise multiply; a custom kernel would be ideal)
         if (!(other instanceof CUDATensor)) throw new IllegalArgumentException("Operand must be a CUDA Tensor");
         CUDATensor<T> o = (CUDATensor<T>) other;
         if (!Arrays.equals(shape, o.shape)) throw new IllegalArgumentException("Shapes mismatch");
    ...
    public Tensor<T> broadcast(int... newShape) {
         // Broadcasting via GPU→CPU roundtrip (requires custom indexing logic)
    ...
    public Tensor<T> transpose(int... permutation) {
         ...
         // 2D matrix transpose via cuBLAS Sgeam/Dgeam
         if (rank() == 2 && permutation[0] == 1 && permutation[1] == 0) {
             ...
             try {
                 ...
                 if (explicitType == Float.class) {
                     ...
                     // Transpose: C(cols x rows) = 1.0 * A^T + 0.0 * C
                     // cuBLAS uses column-major, but our data is row-major.
                     // Row-major A(m,n) is column-major A^T(n,m).
                     // We want row-major result: C_row(n,m) = transpose of A_row(m,n)
                     // In cuBLAS col-major terms: A_col(n,m) transposed => C_col(m,n) = A_col^T
                     float[] alpha = {1.0f};
                     float[] beta = {0.0f};
         ...
         // General N-D transpose: GPU→CPU roundtrip
         if (explicitType == Float.class) {
             float[] src = copyToHostFloat();
    ...
    public Tensor<T> slice(int[] starts, int[] sizes) {
         ...
         for (int i = 0; i < rank(); i++) {
             if (starts[i] < 0 || starts[i] + sizes[i] > shape[i]) {
                 throw new IndexOutOfBoundsException("Slice out of bounds at dim " + i);
             }
         }
    ...
    public Tensor<T> map(Function<T, T> function) {
         // Java lambdas cannot run on GPU; roundtrip through CPU
         if (explicitType == Float.class) {
    ...
    @Override
    public T sum() {
         // Fallback: Copy to host and sum (cuBLAS doesn't have a direct sum operation)
         if (explicitType == Float.class) {
             float[] data = copyToHostFloat();
             float s = 0;
    ...
    public Tensor<T> sum(int axis) {
         ...
         if (newShape.length == 0) {
             // Scalar result wrapped in rank-1
             newShape = new int[]{1};
         }
    ...
    // ========== CPU-side Helpers for Complex Operations ==========
    ...
    private static void broadcastCopy(Object src, Object dst, int[] srcShape, int[] dstShape, int dimOffset) {
        ...
        for (int i = 0; i < dstSize; i++) {
            ...
            if (src instanceof float[]) {
                ((float[])dst)[i] = ((float[])src)[srcIdx];
            } else {
                ((double[])dst)[i] = ((double[])src)[srcIdx];
            }
        }
    }

    private static void transposeGeneric(Object src, Object dst, int[] srcShape, int[] dstShape, int[] perm) {
        int size = Arrays.stream(srcShape).reduce(1, (a, b) -> a * b);
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (int d = 0; d < rank; d++) {
                dstIdx += srcIndices[perm[d]] * dstStrides[d];
            }
            if (src instanceof float[]) {
                ((float[])dst)[dstIdx] = ((float[])src)[i];
            } else {
                ((double[])dst)[dstIdx] = ((double[])src)[i];
            }
        }
    }
    ...
    private static void sliceCopy(Object src, Object dst, int[] srcShape, int[] starts, int[] sizes, int dim, int srcOffset, int dstOffset) {
        if (dim == srcShape.length) {
            if (src instanceof float[]) {
                ((float[])dst)[dstOffset] = ((float[])src)[srcOffset];
            } else {
                ((double[])dst)[dstOffset] = ((double[])src)[srcOffset];
            }
            return;
        }

        int srcStride = 1;
        for (int d = dim + 1; d < srcShape.length; d++) srcStride *= srcShape[d];
        int dstStride = 1;
        for (int d = dim + 1; d < sizes.length; d++) dstStride *= sizes[d];

        for (int i = 0; i < sizes[dim]; i++) {
            sliceCopy(src, dst, srcShape, starts, sizes, dim + 1,
                      srcOffset + (starts[dim] + i) * srcStride,
    ...
    private static void sumAlongAxis(Object src, Object dst, int[] shape, int axis) {
        ...
        for (int i = 0; i < shape.length; i++) {
            if (i != axis) reducedShape[idx++] = shape[i];
        }
        ...
        for (int i = 0; i < size; i++) {
            ...
            // Compute destination index (skip axis dimension)
            int dstIdx = 0;
            int dIdx = 0;
            for (int d = 0; d < shape.length; d++) {
                if (d != axis) {
                    dstIdx += indices[d] * reducedStrides[dIdx];
                    dIdx++;
                }
            }
    ...
    private static Object buildNestedArrayFloat(float[] flat, int[] shape, int dim, int offset) {
        ...
        if (dim == shape.length - 1) {
            return Arrays.copyOfRange(flat, offset, offset + length);
        }
        Object[] result = new Object[length];
        int stride = 1;
        for (int d = dim + 1; d < shape.length; d++) stride *= shape[d];
        for (int i = 0; i < length; i++) {
            result[i] = buildNestedArrayFloat(flat, shape, dim + 1, offset + i * stride);
        }
        return result;
    }
    ...
    private static Object buildNestedArrayDouble(double[] flat, int[] shape, int dim, int offset) {
        ...
        for (int d = dim + 1; d < shape.length; d++) stride *= shape[d];
        for (int i = 0; i < length; i++) {
            result[i] = buildNestedArrayDouble(flat, shape, dim + 1, offset + i * stride);
        }
        return result;
    }
}
