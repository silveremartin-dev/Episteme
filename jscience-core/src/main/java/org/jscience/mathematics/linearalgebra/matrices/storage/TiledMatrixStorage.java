...
public class TiledMatrixStorage implements MatrixStorage<Real> {
    ...
    public Real get(int row, int col) {
        ...
        if (tiles[tileI][tileJ] == null) {
            return org.jscience.mathematics.numbers.real.RealDouble.ZERO;
        }
    ...
    public void set(int row, int col, Real value) {
        ...
        if (tiles[tileI][tileJ] == null) {
            throw new IllegalStateException("Tile not initialized at [" + tileI + "][" + tileJ + "]");
        }
        
        // Note: This assumes tiles are mutable. For immutable tiles, we'd need to replace the entire tile.
